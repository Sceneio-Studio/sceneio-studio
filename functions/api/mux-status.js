import { env, getBody, json, muxAuth, requireUser, supabaseHeaders } from "../_shared.js";
...
export async function onRequestPost(context) {
  try {
    ...
    const uploadResponse = await fetch(`https://api.mux.com/video/v1/uploads/${uploadId}`, { headers: { Authorization: muxAuth(context) } });
    ...
    const assetResponse = await fetch(`https://api.mux.com/video/v1/assets/${encodeURIComponent(upload.asset_id)}`, { headers: { Authorization: muxAuth(context) } });
    ...
    const updateResponse = await fetch(`${env(context, "SUPABASE_URL")}/rest/v1/projects?${filter}`, {
      method: "PATCH",
